...
class UnrealizedPnl(abstract_analysis_evaluator.AnalysisEvaluator):
    PRIORITY: float = 700

    PLOT_UNREALIZED_PORTFOLIO_VALUE_NAME: str = "plot_unrealized_pnl"
    PLOT_UNREALIZED_PORTFOLIO_VALUE_TITLE: str = "Plot unrealized PnL"
    ...
    async def evaluate(
        ...
        if plotted_element is not None:
            unrealized_pnl = await run_data.get_historical_unrealized_pnl()
            for row in unrealized_pnl:
        ...
        for asset, values in y.items():
            plotted_element.plot(
                x=x,
                y=values,
                title=f"Historical unrealized PnL in {asset}",
                x_type="date",
                y_type="log",
                line_shape="hv",
