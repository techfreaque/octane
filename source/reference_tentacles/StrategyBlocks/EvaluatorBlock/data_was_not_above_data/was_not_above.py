...
class DataWasNotAboveDataEvaluator(abstract_evaluator_block.EvaluatorBlock):
    NAME = "data_was_not_above_data"
    TITLE = "Data was not above data"
    TITLE_SHORT = "Data was not above data"
    DESCRIPTION = (
        "Data was not above data allows you to get signals when one data source "
        "was not above the other data source in the near past"
    )
    look_back: int

    def init_block_settings(self) -> None:
        self.activate_single_input_data_node(
            def_val="price_data", data_source_name="Sata source above"
        )
        self.activate_single_input_data_node(
        ...
        self.look_back = self.user_input(
            "was not above x candles back", "int", 10, min_val=1
        )
    ...
    async def execute_block(
        ...
        signals = was_not_above_below.was_not_above(
        ...
        await self.store_evaluator_signals(
            title=f"{below_values_detailed_title} was not above {above_values_detailed_title}",
            signals=signals,
