...
def plot_from_standard_data(data_set: list, plotted_element, title: str):
    ...
    if data_set and len(data_set):
        ...
        for data in data_set:
            ...
            if text is not None:
                text.append(
                    data.get(commons_enums.PlotAttributes.TEXT.value)
                    or data_set[0][commons_enums.PlotAttributes.TEXT.value]
                )
            if color is not None:
                color.append(
                    data.get(commons_enums.PlotAttributes.COLOR.value)
                    or data_set[0][commons_enums.PlotAttributes.COLOR.value]
                )
            if size is not None:
                size.append(
                    data.get(commons_enums.PlotAttributes.SIZE.value)
                    or data_set[0][commons_enums.PlotAttributes.SIZE.value]
                )
            if shape is not None:
                shape.append(
                    data.get(commons_enums.PlotAttributes.SHAPE.value)
                    or data_set[0][commons_enums.PlotAttributes.SHAPE.value]
                )
        plotted_element.plot(
            kind=data_set[0].get(commons_enums.PlotAttributes.KIND.value, None),
            x=x,
            y=y,
            open=open,
            high=high,
            ...
            own_yaxis=data_set[0].get(
                commons_enums.PlotAttributes.OWN_YAXIS.value, False
            ),
            mode=data_set[0].get(commons_enums.PlotAttributes.MODE.value, None),
            color=color,
            size=size,
            symbol=shape,
        )
