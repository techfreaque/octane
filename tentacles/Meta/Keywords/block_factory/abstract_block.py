...
from tentacles.Meta.Keywords.basic_tentacles.matrix_basic_keywords.plottings.plots import (
    plot_conditional,
)
...
from tentacles.Meta.Keywords.basic_tentacles.matrix_basic_keywords.data import (
    write_evaluator_cache,
)
from tentacles.Meta.Keywords.basic_tentacles.matrix_basic_keywords.data.public_exchange_data import (
    get_candles_from_name,
)
...
import typing
from octobot_commons import time_frame_manager
...
from tentacles.Meta.Keywords.block_factory.input_output_nodes.data_input_nodes import (
...
from tentacles.Meta.Keywords.block_factory.input_output_nodes.entry_order_filled_output_node import (
    EntryOrderFilledOutputNode,
)
from tentacles.Meta.Keywords.block_factory.input_output_nodes.entry_orders_input_node import (
...
from tentacles.Meta.Keywords.block_factory.input_output_nodes.exit_orders_output_node import (
    StopLossOutputNode,
    TakeProfitOutputNode,
)
from tentacles.Meta.Keywords.block_factory.input_output_nodes.strategy_start_output_node import (
    StrategyStartOutputNode,
)
from tentacles.Meta.Keywords.pro_tentacles.pro_keywords.indicator_keywords.plotting import (
    normalize_any_time_frame_to_this_time_frame,
)
...
class AbstractBlock:
    ...
    async def get_candles(
        ...
        symbol = symbol or self.block_factory.ctx.symbol
        time_frame = (
            time_frame or (self.custom_time_frame) or self.block_factory.ctx.time_frame
        )
        used_block_factory = block_factory or self.block_factory
        try:
            if self.block_factory.ctx.exchange_manager.is_backtesting:
                return used_block_factory.candles[symbol][time_frame][source_name]
        except KeyError:
            pass
        starting_time = self.block_factory.start_measure_time()
    ...
    def activate_single_input_data_node(
        ...
        self.register_block_input_node(
            input_node_class=SingleIndicatorDataInputNode,
    ...
    def register_take_profit_orders_input(
        ...
        self.register_block_input_node(
            input_node_class=EntryOrderTakeProfitInputNode,
            node_id=self.block_id,
            inputs=self.inputs,
    ...
    def register_stop_orders_input(
        ...
        self.register_block_input_node(
            input_node_class=EntryOrderStopLossInputNode,
            node_id=self.block_id,
            inputs=self.inputs,
    ...
    def register_stop_orders_output(
        ...
        self.register_block_output_node(
            output_node_class=StopLossOutputNode,
            node_id=self.block_id,
            inputs=self.inputs,
    ...
    def register_take_profit_output(
        ...
        self.register_block_output_node(
            output_node_class=TakeProfitOutputNode,
            node_id=self.block_id,
            inputs=self.inputs,
    ...
    def register_order_filled_output(
        ...
        self.register_block_output_node(
            output_node_class=EntryOrderFilledOutputNode,
            node_id=self.block_id,
            inputs=self.inputs,
    ...
    def activate_multiple_input_data_nodes(
        ...
        # TODO handle def_val and other parameters
        self.register_block_input_node(
            input_node_class=MultiIndicatorDataInputNode,
            node_id=self.block_id,
            inputs=self.inputs,
            input_node_title=data_source_name,
            input_node_id=self.last_io_node_id,
    ...
    async def get_input_node_data(
        ...
        if isinstance(node, SingleIndicatorDataInputNode):
            for handle in node.connected_handle_instances.values():
                ...
                try:
                    ...
                    self.block_factory.ctx.logger.exception(
                        error,
                        True,
                        f"Failed to get input node data for {self.NAME}",
                    )
        if get_additional_node_data:
    ...
    async def get_multi_input_node_data(
        ...
        data: typing.List[typing.Tuple[typing.Any, str, str]] = []
        node = next(self.next_indicator_input_generator)
        if isinstance(node, MultiIndicatorDataInputNode):
            for handle in node.connected_handle_instances.values():
                handle: abstract_node.InputOutputNode
                try:
                    await handle.origin_block_instance.execute_block_from_factory(
                    ...
                    data.append((handle.data, handle.chart_location, handle.data_title))
                except Exception as error:
                    self.block_factory.ctx.logger.exception(
                        error,
                        True,
    ...
    def activate_evaluator_signals_input_node(
        ...
        self.register_block_input_node(
            input_node_class=EvaluatorSignalsInputNode,
            node_id=self.block_id,
            inputs=self.inputs,
    ...
    def register_strategy_start_output_node(
        ...
        self.register_block_output_node(
            output_node_class=StrategyStartOutputNode,
            node_id=self.block_id,
            inputs=self.inputs,
            output_node_title=title,
            output_node_id=self.last_io_node_id,
    ...
    async def store_indicator_data(
        ...
        normalized_data = await normalize_any_time_frame_to_this_time_frame(
            self.block_factory,
            data=data,
    ...
    async def plot_and_store_indicator_data(
        ...
        if data_display_conditions is not None:
            ...
            (
                ...
                cutted_data,
                cutted_time_data,
            ) = cut_data_to_same_len((data_display_conditions, data, time_data))
    ...
    async def plot_and_store_signals(
        ...
        (
            ...
            cutted_signals,
        ) = cut_data_to_same_len((times, signal_values, signals))
        values_with_signals = []
        signal_timestamps = []
    ...
    def _get_next_indicator_input_node(
        ...
        for node in self.input_nodes.values():
            if isinstance(
                node, (SingleIndicatorDataInputNode, MultiIndicatorDataInputNode)
            ):
                yield node
    ...
    def __init__(
        ...
        self.next_indicator_input_generator: typing.Iterator[
            typing.Union[SingleIndicatorDataInputNode, MultiIndicatorDataInputNode]
        ] = None
    ...
    def _init_node_in_output(self, inputs: dict) -> str:
        ...
        self.UI.user_input(
            ...
            # title=f"{node['data']['title']} ({node['data']['nodeType']})",
            # editor_options={
    ...
    def _init_node_config(
        ...
        self.UI.user_input(
            ...
            inputs,
            parent_input_name=self.block_id,
            title=f"{node['data']['title']}{title_suffix or ''} ({node['data']['nodeType']})",
            editor_options={
                enums.UserInputEditorOptionsTypes.DISABLE_COLLAPSE.value: True,
