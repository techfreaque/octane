...
class Trader(util.Initializable):
    ...
    async def cancel_open_orders(self, symbol, cancel_loaded_orders=True, side=None,
        ...
        all_cancelled = True
        cancelled_orders = []
        for order in self.exchange_manager.exchange_personal_data.orders_manager.get_open_orders(since=since, until=until, tag=tag, contains_tag=contains_tag):
