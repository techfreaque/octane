...
import tentacles.Meta.Keywords.pro_tentacles.pro_keywords.orders.managed_order_pro.daemons.trailing_stop_loss.trailing_types as trailing_types
    ...
    if (
        ...
        if _open_orders:
            ...
            for order in _open_orders:
                if (
                    order.tag
                    and f"{matrix_constants.TAG_SEPERATOR}{order_group_settings.order_manager_group_id}{matrix_constants.TAG_SEPERATOR}"
                    in order.tag
                ):
                    new_sl_price = None
                    trading_side = (
