...
import tentacles.Meta.Keywords.pro_tentacles.pro_keywords.backtesting.skip_runs as skip_runs
...
class StrategyMakingBaseProducer(
    ...
    async def build_and_trade_strategies_live(self):
        ...
        utilities.end_measure_live_time(
            self.ctx, m_time, " strategy maker - live trading"
        )
    ...
    async def build_strategies_backtesting_cache(self, plot_only_mode=False):
        ...
        self.handle_backtesting_timestamp_whitelist(
            plot_only_mode, self.any_trading_timestamps
        )
        ...
        utilities.end_measure_time(
            s_time,
            f" strategy maker - building strategies for "
    ...
    async def trade_strategies_backtesting(self):
        ...
        for strategy_id, strategy in self.strategies.items():
            if strategy.enable_strategy:
                await strategy.handle_order_daemons(self.ctx, self)
    ...
    async def execute_in_trade_analysis_mode(self):
        ...
        s_time = utilities.start_measure_time(" strategy maker - trade analysis mode")
        self.all_winrates = await trade_analysis_activation.handle_trade_analysis_for_backtesting_first_candle(
            self.ctx, self
        )

        skip_runs.register_backtesting_timestamp_whitelist(self.ctx, [])
        utilities.end_measure_time(s_time, " strategy maker - trade analysis mode")
    ...
    async def init_strategy_maker(self, is_backtesting=False):
        ...
        # await select_history.set_candles_history_size(self.ctx, 1000)
    ...
    async def init_user_input_sections(self):
        ...
        await basic_keywords.user_input(
            ...
            other_schema_values={
                "description": "Use those options wisely when backtesting, "
                "as it will slow down the backtesting speed. Combined with "
                "indicator plotting for backtests, it will be drastically slower. "
    ...
    async def merge_signals_in_backtesting(self):
        strategy_cache = {}
        trades_count = 0
        for strategy_id, strategy_signals in self.strategy_signals.items():
            number_of_indicators = len(strategy_signals.items())
            if number_of_indicators:
                signals = [
                    list(strategy_signals.items())[signal_index][1]
                    for signal_index in range(number_of_indicators)
                ]
                # cut signals to the same length stating from the end
                sig_len = min([len(signal) for signal in signals])
                signals = np.array([signal[-sig_len:] for signal in signals])
                # merge signals
                true_signal = np.prod(signals, axis=0, dtype=bool)
                # filter timestamps
                aux = np.array(self.all_timestamps[-sig_len:], dtype=int)
                strategy_cache[strategy_id] = aux[true_signal]
                trades_count += len(aux[true_signal])

        self.strategy_cache = strategy_cache

        # any signals times
        timestamps_merged = np.unique(
            np.concatenate(list(self.strategy_cache.values()))
        )

        await self.handle_plot_signals()

        return timestamps_merged, trades_count

    async def handle_plot_signals(self):
        if self.plot_signals:
            traiding_keys = [
                self.strategies[strategy_id].trading_side_key
                for strategy_id in self.strategy_cache.keys()
            ]

            # get signals plot values
            if (
                matrix_enums.TradingSideKeys.SHORT in traiding_keys
                or matrix_enums.TradingSideKeys.LONG_EXIT in traiding_keys
            ):
                signal_high = await public_exchange_data.get_candles_(
                    self, matrix_enums.PriceDataSources.HIGH.value
                )
            if (
                matrix_enums.TradingSideKeys.LONG in traiding_keys
                or matrix_enums.TradingSideKeys.SHORT_EXIT in traiding_keys
            ):
                signal_low = await public_exchange_data.get_candles_(
                    self, matrix_enums.PriceDataSources.LOW.value
                )

            # filter and plot signals
            for strategy_id, this_strategy_cache in self.strategy_cache.items():
                idx = [
                    np.where(self.all_timestamps == i)[0][0]
                    for i in this_strategy_cache
                ]
                if self.strategies[strategy_id].trading_side_key == "s":
                    values = signal_high[idx] * 1.001
                else:
                    values = signal_low[idx] * 0.999

                value_key = f"sgnls_s{strategy_id+1}"
                await self.ctx.reset_cached_values([value_key])
                await self.ctx.set_cached_values(
                    values=values,  # signal values
                    cache_keys=this_strategy_cache,  # timestamps
                    value_key=value_key,
                )
                await plotting.plot(
                    self.ctx,
                    f"Strategy {strategy_id+1} Signals",
                    cache_value=value_key,
                    mode="markers",
                    chart="main-chart",
                    color=(
                        "green"
                        if matrix_enums.TradingSideKeys.LONG in traiding_keys
                        else "red"
                    ),
                    size=10,
                )

    def get_signal_while_backtesting(self) -> bool:
        try:
            # TODO add a way to get all strategy signals for a single timestamp
            return [
                strategy_id
                for strategy_id, this_strategy_cache in self.strategy_cache.items()
                if self.ctx.trigger_cache_timestamp in this_strategy_cache
            ]
        except KeyError:
            # no signal(s) stored for this candle
            return []

    def handle_backtesting_timestamp_whitelist(
        self, plot_only_mode, any_trading_timestamps
    ):
        final_whitelist = []
        if plot_only_mode:
