...
def profile():
    ...
    display_intro = (
        flask_util.BrowsingDataProvider.instance().get_and_unset_is_first_display(
            flask_util.BrowsingDataProvider.PROFILE
        )
    )
    exchange_symbols = sorted(
    ...
    return flask.render_template(
        ...
        symbol_list_by_type=models.get_all_symbols_list_by_symbol_type(
            exchange_symbols
        ),
        ...
        other_tentacles_config=models.get_extra_tentacles_config_desc(
            media_url, missing_tentacles
        ),
        config_tentacles_by_group=models.get_tentacles_activation_desc_by_group(
            media_url, missing_tentacles
        ),
...
def _profiles_management(action):
    ...
    if action == "use_as_live":
        ...
        flask.flash(
            f"Profile successfully converted to live profile and selected.", "success"
        )
    ...
    if action == "export":
        ...
        return flask_util.send_and_remove_file(
            file_path, f"{name}_{datetime.now().strftime('%Y%m%d-%H%M%S')}.zip"
        )
...
def _config():
    ...
    if request_data:
        ...
        if (
            constants.TRADING_CONFIG_KEY in request_data
            and request_data[constants.TRADING_CONFIG_KEY]
        ):
        ...
        if (
            constants.TENTACLES_CONFIG_KEY in request_data
            and request_data[constants.TENTACLES_CONFIG_KEY]
        ):
            success = success and models.update_tentacles_activation_config(
        ...
        if (
            constants.EVALUATOR_CONFIG_KEY in request_data
            ...
            success = success and models.update_tentacles_activation_config(
                request_data[constants.EVALUATOR_CONFIG_KEY], deactivate_others
            )
        else:
        ...
        if removed_elements_key in request_data and request_data[removed_elements_key]:
            update_success, err_message = models.update_global_config(
        ...
        if (
            constants.GLOBAL_CONFIG_KEY in request_data
            and request_data[constants.GLOBAL_CONFIG_KEY]
        ):
            success, err_message = models.update_global_config(
    ...
    if flask.request.method == "POST":
        ...
        if flask.request.args:
            ...
            (
                ...
                tentacle_type,
                tentacle_desc,
            ) = models.get_tentacle_from_string(tentacle_name, media_url)
...
def metrics_settings():
    return util.get_rest_reply(
        flask.jsonify(models.activate_metrics(flask.request.get_json()))
    )
...
def beta_env_settings():
    return util.get_rest_reply(
        flask.jsonify(models.activate_beta_env(flask.request.get_json()))
    )
