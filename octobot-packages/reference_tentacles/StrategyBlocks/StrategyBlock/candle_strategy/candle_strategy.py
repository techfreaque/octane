...
class CandleStrategy(abstract_strategy_block.StrategyBlock):
    ...
    async def execute_block(
        ...
        if self.is_allowed_to_trade():
            ...
            # make sure exits get executed before entries
            await self.execute_signals(
                s_time,
                only_on_block_class=abstract_exit_order_block.ExitSignalsOrderBlock,
            )
            await self.execute_signals(
                s_time,
                skip_block_class=abstract_exit_order_block.ExitSignalsOrderBlock,
            )
    ...
    def is_allowed_to_trade(self):
        return (
            self.block_factory.ctx.trigger_source == ActivationTopics.FULL_CANDLES.value
    ...
    async def execute_signals(
        self,
        s_time: float,
        only_on_block_class: typing.Optional[abstract_action_block.ActionBlock] = None,
        skip_block_class: typing.Optional[abstract_action_block.ActionBlock] = None,
    ):
        trades_count = 0
        for strategy_variation in self.strategy_variations_cache:
            if (
                ...
                for action_block in strategy_variation.actions:
                    if (
                        not only_on_block_class
                        or isinstance(action_block, only_on_block_class)
                        and (
                            not skip_block_class
                            or not isinstance(action_block, skip_block_class)
                        )
                    ):
                        if isinstance(
        ...
        for strategy_variation in self.strategy_variations:
            ...
            if len(strategy_variation.actions) and len(strategy_variation.signals):
                ...
                try:
                    ...
                    raise NoTradingSignalsCreatedException
                # filter timestamps
                ...
                if len(variation_cache):
                    ...
                    await self.execute_init_block_data(strategy_variation)
        if self.block_factory.ctx.exchange_manager.is_backtesting:
...
class NoTradingSignalsCreatedException(Exception):
    "Raised when an evaluator doesnt provide trading signals. Signals must be provided always"
