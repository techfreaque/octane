...
class DataIsBelowDataEvaluator(abstract_evaluator_block.EvaluatorBlock):
    NAME = "data_is_below_data"
    TITLE = "Data is below Data"
    TITLE_SHORT = TITLE
    DESCRIPTION = (
        "Data is below Data can be used to get signals when one "
        "data source is below the other source."
    )
    ...
    def init_block_settings(self) -> None:
        self.activate_single_input_data_node(
            data_source_name="Data source below", def_val="ema"
        )
        self.activate_single_input_data_node(
            def_val="price_data",
        ...
        self.below_percent = self.user_input(
            "Needs to be x percent below",
            "float",
            0,
        )
    ...
    async def execute_block(
        ...
        signals = is_above_below.is_below(
        ...
        await self.store_evaluator_signals(
            title=f"{above_values_detailed_title} is below {below_values_detailed_title}",
            signals=signals,
