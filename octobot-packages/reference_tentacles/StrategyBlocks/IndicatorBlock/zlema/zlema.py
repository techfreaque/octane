...
class ZlemaIndicator(abstract_indicator_block.IndicatorBlock):
    NAME = "zlema"
    TITLE = "Zero Lag Exponetial Moving Average"
    TITLE_SHORT = "Zero Lag EMA"
    DESCRIPTION = "Zero Lag Exponetial Moving Average"

    length: int
    candle_source: str

    def init_block_settings(self) -> None:
        self.length = self.user_input("Zero lag EMA length", "int", 50, 0)
        self.candle_source = self.user_select_candle_source_name(
            "Select Zero lag EMA Candle Source"
        )
    ...
    async def execute_block(
        ...
        data = tulipy.zlema(await self.get_candles(self.candle_source), self.length)
        await self.store_indicator_data(
            title=f"{self.TITLE_SHORT} {self.length} {self.candle_source}",
            data=data,
        )
