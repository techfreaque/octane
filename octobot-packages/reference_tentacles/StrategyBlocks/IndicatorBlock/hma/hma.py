...
class HullMovingAverageIndicator(abstract_indicator_block.IndicatorBlock):
    NAME = "hma"
    ...
    def init_block_settings(self) -> None:
        self.length = self.user_input("HMA length", "int", 50, 0)
        self.candle_source = self.user_select_candle_source_name(
            "Select HMA Candle Source", enable_volume=True
        )
    ...
    async def execute_block(
        ...
        data = tulipy.hma(candle_data, self.length)
