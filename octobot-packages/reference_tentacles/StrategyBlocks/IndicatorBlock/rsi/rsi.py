...
class RsiIndicator(abstract_indicator_block.IndicatorBlock):
    NAME = "rsi"
    TITLE = "Relative Strength Index"
    TITLE_SHORT = "RSI"
    DESCRIPTION = "see google for more informations about Relative Strength Index"
    length: int
    candle_source: str

    def init_block_settings(self) -> None:
        self.length = self.user_input("RSI length", "int", 50, 0)
        self.candle_source = self.user_select_candle_source_name(
            "Select RSI Candle Source", enable_volume=True
        )
    ...
    async def execute_block(
        ...
        data = tulipy.rsi(candle_data, self.length)
        await self.store_indicator_data(
            title=f"RSI {self.length} {self.candle_source}",
            data=data,
        )
