...
class VwmaIndicator(abstract_indicator_block.IndicatorBlock):
    NAME = "vwma"
    TITLE = "VWMA"
    TITLE_SHORT = "VWMA"
    DESCRIPTION = "VWMA"
    ...
    def init_block_settings(self) -> None:
        self.length = self.user_input("VWMA length", "int", 50, 0)
        self.candle_source = self.user_select_candle_source_name(
            "Select VWMA Candle Source", enable_volume=True
        )
    ...
    async def execute_block(
        ...
        volume_data = await self.get_candles(matrix_enums.PriceDataSources.VOLUME.value)
        data = tulipy.vwma(candle_data, volume_data, self.length)[self.length :]
