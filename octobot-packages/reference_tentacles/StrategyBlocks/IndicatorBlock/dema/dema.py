...
class DemaIndicator(abstract_indicator_block.IndicatorBlock):
    NAME = "dema"
    TITLE = "DEMA"
    TITLE_SHORT = "DEMA"
    DESCRIPTION = "DEMA"
    ...
    def init_block_settings(self) -> None:
        self.length = self.user_input("DEMA length", "int", 50, 0)
        self.candle_source = self.user_select_candle_source_name(
            "Select DEMA Candle Source", enable_volume=True
        )
    ...
    async def execute_block(
        ...
        data = tulipy.dema(await self.get_candles(self.candle_source), self.length)[
            self.length :
        ]
