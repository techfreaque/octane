...
class SmaIndicator(abstract_indicator_block.IndicatorBlock):
    NAME = "sma"
    ...
    def init_block_settings(self) -> None:
        self.length = self.user_input("SMA length", "int", 50, 0)
        self.candle_source = self.user_select_candle_source_name(
            "Select SMA Candle Source", enable_volume=True
        )
    ...
    async def execute_block(
        ...
        data = tulipy.sma(candle_data, self.length)
